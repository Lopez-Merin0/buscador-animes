...
const renderizarAnimes = (animes) => {
    ...
    animes.forEach((anime, index) => {
        const resourceId = getResourceId(anime) || ''; 
        const columna = document.createElement('div');
        // Aseguramos las clases de columna y la clase 'full-mobile'
        columna.className = 'column is-one-third-desktop is-half-tablet is-full-mobile';

        // Lógica para asignar la clase de color pastel según el estado
        let estadoClass = '';
        if (anime.estado === 'En emisión') {
            estadoClass = 'is-success'; // Menta
        } else if (anime.estado === 'Finalizado') {
            estadoClass = 'is-info'; // Lavanda
        } else if (anime.estado === 'Próximo a estrenar') {
            estadoClass = 'is-warning'; // Amarillo
        }

        // Estructura HTML corregida para el centrado vertical (usa 'anime-card' y 'anime-avatar')
        columna.innerHTML = `
            <div class="card anime-card">
                <div class="card-content">
                    
                    <img class="anime-avatar" 
                         src="${anime.imagen || 'URL_IMAGEN_POR_DEFECTO'}" 
                         alt="Portada de ${anime.titulo || ''}">
                    
                    <p class="title is-4 card-title">${anime.titulo || ''}</p>
                    
                    <div class="tags is-centered">
                        <span class="tag is-primary">${anime.genero || ''}</span>
                        <span class="tag ${estadoClass}">${anime.estado || ''}</span>
                    </div>
                </div>
                
                <footer class="card-footer">
                    <a href="#" class="card-footer-item button is-small is-primary editar-anime" data-id="${resourceId}" data-index="${index}">
                        <span class="icon"><i class="fas fa-edit"></i></span> Editar
                    </a>
                    <a href="#" class="card-footer-item button is-small is-danger eliminar-anime" data-id="${resourceId}" data-index="${index}">
